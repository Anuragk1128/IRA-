...
export async function fetchBrandProductsByCategorySubcategory(
  ...
  return activeProducts.map((it: any): Product => {
    ...
    return {
      ...
      taxAmount: typeof it.taxAmount === 'number' ? it.taxAmount : (it.taxAmount ? Number(it.taxAmount) : null),
      priceIncludingTax: typeof it.priceIncludingTax === 'number' ? it.priceIncludingTax : (it.priceIncludingTax ? Number(it.priceIncludingTax) : null),
      gstRate: typeof it.gstRate === 'number' ? it.gstRate : (it.gstRate ? Number(it.gstRate) : undefined),
